    ...
    <div className={cn('w-full', className)}>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8 lg:gap-0">
        {steps.map((step, index) => {
          // Explicitly check if we're on the mid-year step (step 4)
          const isMidYearStep = currentStep === 4;
          
          // Mark step 3 (Review) as completed when on mid-year step
          const isCompleted = 
            step.number < currentStep ||  // Normal completion rule
            (step.number === 3 && isMidYearStep); // Special case for Review step
